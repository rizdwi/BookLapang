        ...
        <form action="{{ route('booking.store') }}" method="POST" class="p-6 sm:p-8">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                        ...
                        <span class="font-bold text-gray-900 text-base">
                            {{ \Carbon\Carbon::parse($slot->tanggal)->translatedFormat('l, d F Y') }}
                        </span>
                        <span class="text-xs font-semibold text-teal-700 block mt-0.5">
                            Pukul {{ substr($slot->jam_mulai, 0, 5) }} - {{ substr($slot->jam_selesai, 0, 5) }} WIB (1 Jam)
                        </span>
                    </div>
                </div>
            ...
            <div class="mb-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
                    {{-- QRIS --}}
                    <label class="border rounded-lg p-3 flex flex-col cursor-pointer transition-all"
                    ...
                    <div class="flex flex-col sm:flex-row items-center gap-6">
                        ...
                        <div class="text-left w-full space-y-2 text-sm">
                            ...
                            <div>
                                ...
                                <span class="font-black text-emerald-700 text-lg">Rp {{ number_format($lapangan->harga_per_jam, 0, ',', '.') }}</span>
                            </div>
                            <div class="bg-teal-50 text-teal-800 text-xs p-2.5 rounded-lg border border-teal-200">
                                <strong>Panduan:</strong> Buka aplikasi m-Banking atau E-Wallet pilihan Anda (GoPay, OVO, Dana, BCA Mobile), scan QR code di atas, dan saldo akan terverifikasi secara otomatis.
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="bg-gray-50 p-3 rounded-lg border border-gray-200 text-xs text-gray-600 space-y-1">
                            ...
                            <ol class="list-decimal list-inside space-y-0.5 pl-1">
                                ...
                                <li>Periksa nominal tagihan sebesar <strong>Rp {{ number_format($lapangan->harga_per_jam, 0, ',', '.') }}</strong>, lalu konfirmasi PIN Anda.</li>
                            </ol>
                        </div>
                    </div>
                </div>
                ...
                <div x-show="paymentMethod === 'cash'" x-transition class="bg-white border border-gray-200 rounded-xl p-5 shadow-sm mb-6" style="display: none;">
                    <div class="text-sm text-gray-700 space-y-2">
                        ...
                        <p class="text-xs text-gray-600">
                            Silakan datang ke kasir lapangan <strong>{{ $lapangan->nama }}</strong> paling lambat 15 menit sebelum waktu pemesanan dimulai. Tunjukkan ID booking Anda kepada petugas untuk melunasi biaya sewa.
                        </p>
                    </div>
                </div>
            </div>
