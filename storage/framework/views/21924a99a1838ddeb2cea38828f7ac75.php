...
<div class="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4" 
    ...
    <div>
        ...
        <p class="text-sm text-[#64748b]">Selamat datang kembali, <span class="font-semibold text-gray-800"><?php echo e(auth()->user()->name); ?></span>! Pantau status reservasi lapanganmu di sini.</p>
    </div>
    <div>
        <a href="<?php echo e(route('home')); ?>" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-semibold rounded-lg text-white bg-[#0d9488] hover:bg-[#0f766e] shadow-sm transition-colors">
            + Pesan Lapangan Baru
        </a>
    </div>
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
    <div class="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
        ...
        <span class="text-2xl font-black text-[#1e3a5f] mt-1 block"><?php echo e($totalBookings ?? 0); ?></span>
    </div>

    <div class="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
        <span class="text-xs font-semibold text-amber-600 uppercase tracking-wider block">Menunggu Pembayaran</span>
        <span class="text-2xl font-black text-amber-600 mt-1 block"><?php echo e($pendingBookings ?? 0); ?></span>
    </div>

    <div class="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
        <span class="text-xs font-semibold text-teal-600 uppercase tracking-wider block">Dikonfirmasi</span>
        <span class="text-2xl font-black text-teal-600 mt-1 block"><?php echo e($confirmedBookings ?? 0); ?></span>
    </div>

    <div class="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
        <span class="text-xs font-semibold text-emerald-600 uppercase tracking-wider block">Selesai</span>
        <span class="text-2xl font-black text-emerald-600 mt-1 block"><?php echo e($doneBookings ?? 0); ?></span>
    </div>
</div>
                    ...
                    <?php $__currentLoopData = $bookings; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $booking): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="font-bold text-[#1e3a5f] block">#BK-<?php echo e(str_pad($booking->id, 4, '0', STR_PAD_LEFT)); ?></span>
                                <span class="text-gray-900 font-medium block mt-0.5"><?php echo e($formattedDate); ?></span>
                                <span class="text-xs text-teal-700 font-semibold block">
                                    <?php echo e(substr($booking->jam_mulai, 0, 5)); ?> - <?php echo e(substr($booking->jam_selesai, 0, 5)); ?> WIB
                                </span>
                            </td>

                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <?php if($booking->lapangan && $booking->lapangan->foto): ?>
                                        <img src="<?php echo e(asset('storage/' . $booking->lapangan->foto)); ?>" class="w-10 h-10 rounded-lg object-cover border border-gray-200" alt="">
                                    <?php else: ?>
                                        <div class="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center text-xs text-gray-400">No Img</div>
                                    <?php endif; ?>
                                    <div>
                                        <span class="font-bold text-gray-900 block"><?php echo e($booking->lapangan->nama ?? 'Lapangan'); ?></span>
                                        <span class="text-xs text-gray-500 block"><?php echo e(ucfirst($booking->lapangan->tipe ?? '-')); ?></span>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="uppercase text-xs font-bold text-gray-700 bg-gray-100 px-2 py-1 rounded">
                                    <?php echo e(str_replace('_', ' ', $booking->metode_pembayaran ?? 'qris')); ?>

                                </span>
                            </td>

                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="font-black text-[#1e3a5f] text-base"><?php echo e($formattedTotal); ?></span>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-xs font-semibold">
                                <?php if($booking->status === 'pending'): ?>
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <button type="button" 
                                                @click="openPayment('<?php echo e(str_pad($booking->id, 4, '0', STR_PAD_LEFT)); ?>', '<?php echo e($booking->lapangan->nama); ?>', '<?php echo e($formattedTotal); ?>', '<?php echo e($booking->metode_pembayaran ?? 'qris'); ?>', '<?php echo e($vaNum); ?>', '<?php echo e($formattedDate); ?>')"
                                                class="text-white bg-[#0d9488] hover:bg-[#0f766e] px-2.5 py-1.5 rounded font-bold shadow-sm transition-colors">
                                            Bayar Sekarang
                                        </button>

                                        
                                        <form action="<?php echo e(route('booking.cancel', $booking->id)); ?>" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin membatalkan booking ini?');">
                                            <?php echo csrf_field(); ?>
                                ...
                                <?php else: ?>
                                    <a href="<?php echo e(route('lapangan.show', $booking->lapangan_id)); ?>" class="text-[#0d9488] hover:text-[#0f766e] bg-teal-50 hover:bg-teal-100 px-3 py-1.5 rounded transition-colors">
                                        Pesan Lagi
                                    </a>
                                <?php endif; ?>
                            </td>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Rizz\Documents\BookLapang\resources\views/customer/dashboard.blade.php ENDPATH**/ ?>